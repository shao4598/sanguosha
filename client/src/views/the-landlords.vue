        ...
        >
          <el-form label-width="120px" :model="formRecordsGames">
            <el-form-item label="结果">
              ...
              >
                <el-radio-button label="1" border>胜</el-radio-button>
                <el-radio-button label="0" border>负</el-radio-button>
              </el-radio-group>
              <el-checkbox
                border
                true-label="1"
            ...
            <el-form-item label="房间">
              <el-radio-group v-model="formRecordsGames.room">
                <el-radio-button border label="0">普通场</el-radio-button>
                <el-radio-button border label="1">至尊场</el-radio-button>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="倍数">
              <el-radio-group v-model="formRecordsGames.multiple">
                <el-radio-button border label="3">3倍</el-radio-button>

                <el-radio-button border label="2">2倍</el-radio-button>
                <el-radio-button border label="1">1倍</el-radio-button>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="地主">
              ...
              <el-col :span="12">
                <el-radio border label="0" v-model="formRecordsGames.role">
                  本人
                </el-radio>
              </el-col>
            </el-form-item>
            <el-form-item label="农民1">
              ...
              <el-col :span="12">
                <el-radio border label="1" v-model="formRecordsGames.role">
                  本人
                </el-radio>
              </el-col>
            </el-form-item>
            <el-form-item label="农民2">
              ...
              <el-col :span="12">
                <el-radio border label="2" v-model="formRecordsGames.role">
                  本人
                </el-radio>
              </el-col>
            </el-form-item>
          ...
          <div slot="footer" class="dialog-footer">
            <el-button type="primary" @click="onSubmitRecordsGames()">
              确 定
            </el-button>
            <el-button @click="dialogFormGamesVisible = false">取 消</el-button>
          </div>
        </el-dialog>
      </el-col>
    </el-header>
      ...
      <el-col :span="5">
        <el-form label-width="50px" :model="formSearch" style="width: 90%">
          ...
          <el-form-item label="玩家">
            <el-radio-group v-model="formSearch.player" size="small">
      ...
      <el-col :span="14">
        <el-table :data="recordsGames" stripe tooltip-effect="dark">
          ...
          <el-table-column
            ...
            sortable
          ></el-table-column>
          <el-table-column prop="isWin" label="结果" width="120" sortable>
            <template slot-scope="scope">
              <el-tag :type="scope.row.color" size="mini" effect="dark">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button
                size="mini"
                @click="onDeleteRecordsGames(scope.$index, scope.row)"
              >
                删除
              </el-button>
            </template>
        ...
        <el-row style="margin-bottom: 20px">
          ...
          <el-col :span="14">
            ...
            <el-dialog
              ...
              :close-on-click-modal="false"
            >
              <el-form label-width="120px" :model="formRecordsGolds">
              ...
              <div slot="footer" class="dialog-footer">
                <el-button type="success" @click="onSubmitRecordsGolds()">
                  确 定
                </el-button>
                <el-button @click="dialogFormGoldsVisible = false">
                  取 消
                </el-button>
              </div>
            </el-dialog>
              ...
              <el-table :data="recordsGolds" height="500">
                ...
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    <el-button
                      size="mini"
                      @click="onDeleteRecordsGolds(scope.$index, scope.row)"
                    >
                      删除
                    </el-button>
                  </template>
        ...
        <el-row style="margin-bottom: 20px">
          ...
          <el-col :span="14">
            ...
            <el-dialog
              ...
              center
            >
              <el-form label-width="120px" :model="formRecordsBeans">
              ...
              <div slot="footer" class="dialog-footer">
                <el-button type="warning" @click="onSubmitRecordsBeans()">
                  确 定
                </el-button>
                <el-button @click="dialogFormBeansVisible = false">
                  取 消
                </el-button>
              </div>
            </el-dialog>
              ...
              <el-table :data="recordsBeans" height="500">
                ...
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    <el-button
                      size="mini"
                      @click="onDeleteRecordsBeans(scope.$index, scope.row)"
                    >
                      删除
                    </el-button>
                  </template>
  ...
  methods: {
    ...
    onDeleteRecordsGames(index, row) {
      this.$confirm('是否确认删除?', '提示', {
        ...
        .then(() => {
          axios
            .delete(this.serverUrl + '/records/games/' + this.token, {
              params: {
                guid: row.guid,
                golds: row.golds,
                beans: row.beans,
    ...
    onDeleteRecordsGolds(index, row) {
      this.$confirm('是否确认删除?', '提示', {
        ...
        .then(() => {
          axios
            .delete(this.serverUrl + '/records/golds/' + this.token, {
              params: {
                guid: row.guid,
                golds: row.count,
              },
    ...
    onDeleteRecordsBeans(index, row) {
      this.$confirm('是否确认删除?', '提示', {
        ...
        .then(() => {
          axios
            .delete(this.serverUrl + '/records/beans/' + this.token, {
              params: {
                guid: row.guid,
                beans: row.count,
              },
    ...
    onSubmitRecordsGolds() {
      axios
        .post(
          this.serverUrl + '/records/golds/' + this.token,
          this.formRecordsGolds,
        )
        .then(({ data: { status } }) => {
          if (status !== '0') {
            return;
    ...
    onSubmitRecordsBeans() {
      axios
        .post(
          this.serverUrl + '/records/beans/' + this.token,
          this.formRecordsBeans,
        )
        .then(({ data: { status } }) => {
          if (status !== '0') {
            return;
    ...
    getIncome() {
      axios
        .get(this.serverUrl + '/records/income/' + this.token, {
          params: this.datePickerRangeUnix,
        })
        .then(({ data: { status, data } }) => {
          if (status !== '0') {
